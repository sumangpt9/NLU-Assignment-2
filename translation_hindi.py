...
#/commonCrawl
text_file = open("data/hindencorp05.plaintext", "r")
C_eng_hin = text_file.readlines()
...
for i in range(40000):
    l=list()
    for word in C_eng_hin[i].split("\t"):
        l.append(word)

    pairs.append( [normalizeString(str(l[n-2])) ,normalizeString(str(l[n-1]))])
...
tokenized_corpus_hin=[]

for i in range(len(pairs)):
    
    tokenized_corpus_eng.append(pairs[i][0])
    tokenized_corpus_hin.append(pairs[i][1])
...
vocabulary_hin = ["SOS","EOS","UNK"]
tokens_hin=[]
for sentence in tokenized_corpus_hin:
    
    
    for token in sentence.split(" "):
        tokens_hin.append(token)
        #if token.isnumeric():
            #tokens.append("num")
        if token not in vocabulary_hin:
            vocabulary_hin.append(token)
...
word2idx_hin = {w: idx for (idx, w) in enumerate(vocabulary_hin)}
idx2word_hin = {idx: w for (idx, w) in enumerate(vocabulary_hin)}

vocabulary_size_hin = len(vocabulary_hin)
...
def indexesFromSentence(lang, sentence):
    ...
    if(lang=='eng'):
        ...
        for word in sentence.split(' '):
            if(word in vocabulary_hin):
                l.append(word2idx_hin[word])
            else:
                l.append(word2idx_hin["UNK"])
        #return [word2idx_eng[word] for word in sentence.split(' ')]
...
def tensorsFromPair(pair):
    ...
    target_tensor = tensorFromSentence("hin", pair[1]).to(device)
...
def trainIters(encoder, decoder, n_iters, print_every=1, plot_every=40000, learning_rate=0.001):
    ...
    for di in range(max_length):
       # print(decoder_output.size(),decoder_context.size(), decoder_hidden.size(), decoder_attention.size())
        ...
        if ni == EOS_token:
            ...
            #print(ni)
            decoded_words.append(idx2word_hin[ni.item()])
...
if(attn=="multiplicative"):
  decoder = MultAttnDecoderRNN(hidden_size, vocabulary_size_hin)
elif(attn=="scaledDotProd"):
  decoder = ScaledDotProdAttnDecoderRNN(hidden_size, vocabulary_size_hin)
elif(attn=="Bahadanau"):
  decoder = BahdanauAttnDecoderRNN(hidden_size, vocabulary_size_hin)
...
source_file = open("data/newstest2014-hien-src.en.sgm", "r")
target_file = open("data/newstest2014-hien-src.hi.sgm", "r")
